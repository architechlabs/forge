...
The add-on creates real Home Assistant helper entities through a managed package file and keeps them synchronized through the Home Assistant API. For device-native behavior, the target instance still needs the relevant integration configured, such as KNX, WiZ, MQTT, ZHA, Z-Wave JS, ESPHome, Matter, Shelly, Hue, or Tuya.
...
- Managed YAML package generation under `/homeassistant/packages`.
